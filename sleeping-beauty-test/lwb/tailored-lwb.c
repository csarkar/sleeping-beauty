...
static strobe_data_struct  strobe_packet;
...
static uint16_t STROBE_SLOTS, strobe_slots;
...
static uint16_t	max_node_count;
static uint8_t  stat_index;
/*---------------------------------------------------------------------------*/
...
static inline int8_t reset_parameters() {
	...
	if(get_rx_cnt() == 0) {
		...
		DATA_SLOTS		 = sync_data.data_slots%ACTIVE_SELECTION;
		if(DATA_SLOTS > max_node_count) {
			max_node_count = DATA_SLOTS+1;
		}
		
		if(sync_data.data_slots < ACTIVE_SELECTION && DATA_SLOTS) {
			STROBE_SLOTS	 = max_node_count;
		} else {
			STROBE_SLOTS	 = 0;
		}
...
void filled_superframe() {
	...
	if(run_time < TRAINING_PERIOD) {
		...
		if(run_time%(IPI*ACTIVE_SELECTION_ROUND) == 0) {
			choose_active_nodes(SINK_NODE_ID, sync_data.slot_vector);
			sync_data.data_slots = ACTIVE_SELECTION + max_data_slots;
			sync_data.sleep_slots = IPI*(ACTIVE_SELECTION_ROUND-1);
		} else {
			for(i=0; i<max_data_slots; i++) {
				sync_data.slot_vector[i/8] |= 1 << (i%8);
			}
			sync_data.data_slots  = max_data_slots;
			sync_data.sleep_slots = IPI;
		}
	}
}
 ...
 * 
 */
void bare_superframe(uint8_t rr_slots, uint8_t sleep_duration) {
	sync_data.rr_slots    = rr_slots;
...
void prepare_next_superframe() {
	...
	if(run_time < COOLOFF_PERIOD) {
		...
		bare_superframe(MAX_RR_SLOTS_P_SECOND, 1);				
	} else if(run_time%IPI == 0) {
		/* fill the superframe with suitable RR, Data and sleep slots */
		filled_superframe();
	} else {
...
void next_radio_activity_schedule(struct rtimer *t, void *ptr) {
	...
	else if(strobe_slots < STROBE_SLOTS) {
		NEXT_SLOT = (rtimer_clock_t)(NEXT_SLOT + STROBE_SLOT_LEN);
		if (IS_SINK()) {
			if(strobe_slots==0) {
				GRACE_PERIOD = 4*TX_GUARD_TIME;
			}
		} else {
			if(flow_info.slot == strobe_slots) {
				GRACE_PERIOD = TX_GUARD_TIME;
			}
		}
	}
	else if(data_slots < DATA_SLOTS) {
...
void prepare_data() {
	...
	if(decide_participation(data_slots)){
		if(flow_info.slot == data_slots) {
			...
			sensed_data.etx       = own_data.etx;
			sensed_data.parent[0] = own_data.parent[0];
			sensed_data.parent[1] = own_data.parent[1];
...
void process_data() {
	...
	if(sensed_data.dst == node_id) {
		...
		node_info[sensed_data.src-1].parent[0] = sensed_data.parent[0];
		node_info[sensed_data.src-1].parent[1] = sensed_data.parent[1];
	}
	...
	if(rr_slots%MIN_RR_SLOTS == 1) {
		if(!get_rx_cnt()) {
			...
			if(IS_SINK()) {
				...
				cluster_member_notification(req_reply.dst, req_reply.slot);
			}
		}
	} 
	else if(rr_slots%MIN_RR_SLOTS == 2) {
 ...
 * 
 */
void prepare_strobe_data() {
	
	if (IS_SINK()) {
		if(strobe_slots == 1) {
			strobe_packet.src = SINK_NODE_ID;
			strobe_packet.etx = 0;
			FLOODING_ROLE = GLOSSY_INITIATOR;
		} else {
			FLOODING_ROLE = GLOSSY_NO_FLOODING;
		...
		if(flow_info.slot == strobe_slots-1) {
			strobe_packet.src = node_id;
			strobe_packet.etx = own_data.etx;
			FLOODING_ROLE = GLOSSY_INITIATOR;
		} else if(DISCOVERY_BOOTSTRAPPING() || parent_stat[stat_index].slot == strobe_slots) {
			strobe_packet.src = 0;
			strobe_packet.etx = INITIAL_ETX;
			FLOODING_ROLE = GLOSSY_RECEIVER;
		} else{
			FLOODING_ROLE = GLOSSY_NO_FLOODING;
		}
 ...
 * 
 */
void process_strobe_data() {
	if(FLOODING_ROLE == GLOSSY_RECEIVER) {
		if(parent_stat[stat_index].slot == strobe_slots && parent_stat[stat_index].etx != 0) {
			update_neighbor_stat(stat_index, strobe_packet.src, strobe_packet.etx, get_rx_cnt(), (int8_t)S_TX);
			stat_index++;
		} else if(DISCOVERY_BOOTSTRAPPING() && get_rx_cnt() && strobe_packet.etx < own_data.etx+50) {
			add_neighbor_stat(strobe_slots, strobe_packet.src, strobe_packet.etx, get_rx_cnt(), (int8_t)S_TX);
		}
		
		if(strobe_slots == STROBE_SLOTS) {
			if(DISCOVERY_BOOTSTRAPPING()) {
				discovery_cnt++;
			}
		#if ALTERNATE_PARENT
		...
		#endif
		/*#if ENERGEST_CONF_ON
			l2 = energest_type_time(ENERGEST_TYPE_LISTEN);
			t2 = energest_type_time(ENERGEST_TYPE_TRANSMIT);
		#endif*/
		}
	}
	
	/* at the end of strobe slots */
	if(strobe_slots == STROBE_SLOTS) {
		STROBE_SLOTS = 0;
	}
}
...
char tailored_lwb_scheduler(struct rtimer *t, void *ptr) {
	...
	while (1) {
		if(SYNC_SLOT) {
		...
		else if(STROBE_SLOTS) {
			/* account for how many probe slots have elapsed */
			strobe_slots++;
			
			/* prepare the packet for the probe slot */
			prepare_strobe_data();
			
			if(FLOODING_ROLE != GLOSSY_NO_FLOODING) {
				rtimer_clock_t t_stop = RTIMER_TIME(t) + STROBE_SLOT_DURATION;
				tailored_glossy_start((uint8_t *)&strobe_packet, STROBE_LEN, FLOODING_ROLE, STROBE, GLOSSY_NO_SYNC, S_TX,
							APPLICATION_HEADER, t_stop, (rtimer_callback_t)tailored_lwb_scheduler, t, ptr);
				PT_YIELD(&pt);
				tailored_glossy_stop(&src, &dst);
			}
			...
			/* process the data received in the probe slot */
			process_strobe_data();			
		}
		else if(DATA_SLOTS) {
			...
			if(SLEEP_SLOTS && SLEEP_SLOTS%IPI == 0) {
				// if a few nodes are active, every IPI slot enable data 
				// transmission from the active nodes until next sync slot arrives
				data_slots = 0;
				DATA_SLOTS = sync_data.data_slots%ACTIVE_SELECTION;
